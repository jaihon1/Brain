...
RUN_NAME = "Run 1 epochs = 1000"
learning_rate = 0.001
training_epochs = 1000

# Define how many inputs and outputs are in our neural network
number_of_inputs = 26
number_of_outputs = 194
...
with tf.variable_scope('cost'):
    Y = tf.placeholder(tf.float32, shape=(None, number_of_outputs), name="Y")
...
with tf.Session() as session:
    ...
    for epoch in range(1):
            predicted_data = session.run(prediction, feed_dict={X: X_scaled_training, Y: Y_scaled_training})

            for i in range(194):
                df_max = predicted_data[i].argmax()
                print(df_max)
